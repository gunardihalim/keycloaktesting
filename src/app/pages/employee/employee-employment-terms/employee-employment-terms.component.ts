...
@Component({
    selector:"employee-employment-terms",
    templateUrl:"employee-employment-terms.component.html",
...
export class EmployeeEmploymentTermsComponent implements OnInit, AfterViewInit {
...
modelContractStartDate:Date;
modelContractEndDate:Date;

modelEmploymentType:any;
public arrEmploymentType:string[] = ['Contract','Permanent'];
...
aktif_error:boolean=false;
    ...
    ngOnInit(){
        ...
        this.methodServices.getUrlApi('/api/admin/employmentterms?employeeid=' + this.employeeId,
        localStorage.getItem(this.methodServices.seasonKey),
        (result)=>{
            if (result == 'Error'){
                this.aktif_error = true;
                this.router.navigate(['/employee/terms/error'],{queryParamsHandling:'merge'})
                return
            }
            if (typeof(result) == 'object'){
                this.modelEmploymentType = result.empStatus;
                if (result.empStatus == 'Contract'){
                    let startDateConvert = new Date(result.startDate)
                    this.modelContractStartDate = startDateConvert

                    let endDateConvert = new Date(result.endDate)
                    this.modelContractEndDate = endDateConvert
                }
            }
        })
        
        setTimeout(()=>{
            var selectrType:any = document.getElementById('selectrEmpType')
            var options = {searchable:true}
            var selectEmpType = new Selectr(selectrType,options)
        },500)
    }
    ...
    ngAfterViewInit(){
        ...
        //Employment Terms
        // this.modelEmploymentType = 'Ex-Employee'
