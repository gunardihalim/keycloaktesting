...
export class EmployeeEmploymentAddressComponent implements OnInit {
...
modelAddressType:string;
modelAddress:string;
modelPostalCode:string;
modelTownCity:string;
modelProvince:string;
modelCountry:string;
photoProfile:any;
modelPrimary:boolean;
addressTypeList:any[] = ['KTP Address'];
...
aktif_error:boolean = false;
    ...
    ngOnInit(){
        ...
        this.methodServices.getUrlApi('/api/admin/listaddress?employeeid=' + this.employeeId,
            localStorage.getItem(this.methodServices.seasonKey),
            (result)=>{
            if (result == 'Error'){
                this.aktif_error = true;
                this.router.navigate(['/employee/address/error'],{queryParamsHandling:'merge'})
                return
            }
            if (typeof(result) == 'object')
            {
                for(var key in result){
                    this.modelAddressType = result[key].type
                    this.modelAddress = result[key].address
                    this.modelPrimary = result[key].primaryFlag == "Yes" ? true : false
                    this.modelPostalCode = result[key].zipCode
                    this.modelTownCity = result[key].city
                    this.modelProvince = result[key].province
                    this.modelCountry = result[key].country
                }
