...
@Component({
    selector:"work-structure-job-list",
    templateUrl:"work-structure-job-list.component.html",
...
export class WorkStructureJobListComponent implements OnInit {
  
arrBreadCrumb = ['Work Structure','Job']
...
@ViewChild('tableJob') table
    ...
    ngOnInit(){
        ...
        // this.methodServices.getUrlApi('/api/job/find',
    ...
    onSelect({selected}) {
      ...
      this.selected.push(...selected);
       if (this.selected.length > 0){
         ...
         this.aktif_table = false
        //  this.urlEndParse = 'JOB'
         this.router.navigate(['/work-structure/job','job-form'],
                {queryParams:{id:id,
  ...
  activeStatusProc(){
    ...
    backTable(event){
      if (event.toLowerCase() == 'work structure' || 
          event.toLowerCase() == 'job'){
            this.router.navigate(['/work-structure/job'])
